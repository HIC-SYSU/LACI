...
# __all__ = ["sliding_window_inference_text"]
...
def sliding_window_inference_text(
    inputs: torch.Tensor | MetaTensor,
    inputs_text: torch.Tensor | MetaTensor,
    roi_size: Sequence[int] | int,
    sw_batch_size: int,
    predictor: Callable[..., torch.Tensor | Sequence[torch.Tensor] | dict[Any, torch.Tensor]],
    overlap: Sequence[float] | float = 0.25,
    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        if sw_batch_size > 1:
            ...
            win_data = inputs[unravel_slice[0]].to(sw_device)
        if with_coord:
            seg_prob_out = predictor(win_data, inputs_text, unravel_slice, *args, **kwargs)  # batched patch
        else:
            seg_prob_out = predictor(win_data, inputs_text, *args, **kwargs)  # batched patch
...
def sliding_window_inference_text_device(
    inputs: torch.Tensor | MetaTensor,
    inputs_text: torch.Tensor | MetaTensor,
    roi_size: Sequence[int] | int,
    sw_batch_size: int,
    ...
    **kwargs: Any,
    ...
    for slice_g in tqdm(windows_range) if progress else windows_range:
        ...
        if sw_batch_size > 1:
            ...
            win_data = inputs[unravel_slice[0]].to(sw_device)
        if with_coord:
            seg_prob_out = predictor(win_data, inputs_text, device,unravel_slice, *args, **kwargs)  # batched patch
        else:
            seg_prob_out = predictor(win_data, inputs_text, *args, **kwargs)  # batched patch
