...
load_weights_path = "/data/chenjinfeng/CKF/CKF/checkpoints/mrct_5_model_state_epoch_6"
savepath = "/data/chenjinfeng/CKF/CKF/test_result/Epoch6"
...
def parse_arguments(args):
    ...
    #data
    parser.add_argument("--data_path", type=str, default='/data/chenjinfeng/Data/CKF/MRCT', help='Path to ACDC dataset')
...
if __name__ == "__main__":
    ...
    for CT_patients in CT_test_list:
        ...
        x_list_CT = np.squeeze(np.concatenate((np.arange(0, w_CT - args.patch_size, args.patch_size // args.spacing)[:, np.newaxis],
                                            np.array([w_CT - args.patch_size])[:, np.newaxis])).astype(int))
        y_list_CT = np.squeeze(np.concatenate((np.arange(0, h_CT - args.patch_size, args.patch_size // args.spacing)[:, np.newaxis],
                                            np.array([h_CT - args.patch_size])[:, np.newaxis])).astype(int))

        if d_CT <= args.patch_size:
            z_list_CT = [0]
        else:
            z_list_CT = np.squeeze(np.concatenate((np.arange(0, d_CT - args.patch_size, args.patch_size // 4)[:, np.newaxis],
                                                np.array([d_CT - args.patch_size])[:, np.newaxis])).astype(int))
        ...
        for z in z_list_CT:
            for x in x_list_CT:
                for y in y_list_CT:
                    ...
                    # print("patch_tensor:{}".format(patch_tensor.shape))
        ...
        nib.save(predict_nii_CT,
                 os.path.join(savepath, CT_patients.split('.')[0], 'predict.nii.gz'))
        # nib.save(predict_nii, os.path.join(save_path, '{}/predict.nii.gz'.format(test_list[idx])))

        t += 1
        print("length:{}".format(t))
        print("{} Val_dice_endo  {:.3f}, Val_dice_epi  {:.3f},  Dice: {:.3f}".format(CT_patients, dice[1], dice[2],
                                                                                  (dice[1] + dice[2]) / 2))
        print(" Val_iou_endo  {:.3f}, Val_iou_epi  {:.3f},  IOU: {:.3f}".format( iou[1], iou[2],
                                                                                     (iou[1] + iou[2]) / 2))
        print(" Val_hd_endo  {:.3f}, Val_hd_epi  {:.3f},  HD: {:.3f}".format( hd[1], hd[2],
                                                                                     (hd[1] + hd[2]) / 2))
        print(" Val_rmse_endo  {:.3f}, Val_rmse_epi  {:.3f},  RMSE: {:.3f}".format(rmse[1], rmse[2],
                                                                                     (rmse[1] + rmse[2]) / 2))
        print(" Val_rvd_endo  {:.3f}, Val_rvd_epi  {:.3f},  RVD: {:.3f}".format(rvd[1], rvd[2],
                                                                                (rvd[1] + rvd[2]) / 2))
    ...
    print("ALL CT :dice_endo  {:.3f}  {:.6f}, Val_dice_epi  {:.3f}  {:.6f}".format(np.average(CT_all_dice_endo),np.var(CT_all_dice_endo),
                                                                   np.average(CT_all_dice_epi),np.var(CT_all_dice_epi)))
    print("iou_endo  {:.3f}  {:.6f}, Val_iou_epi  {:.3f}  {:.6f}".format(np.average(CT_all_iou_endo),
                                                                                   np.var(CT_all_iou_endo),
                                                                                   np.average(CT_all_iou_epi),
                                                                                   np.var(CT_all_iou_epi)))
    print("hd_endo  {:.3f}  {:.6f}, Val_hd_epi  {:.3f}  {:.6f}".format(np.average(CT_all_hd_endo),
                                                                                   np.var(CT_all_hd_endo),
                                                                                   np.average(CT_all_hd_epi),
                                                                                   np.var(CT_all_hd_epi)))
    print("rmse_endo  {:.3f}  {:.6f}, Val_rmse_epi  {:.3f}  {:.6f}".format(np.average(CT_all_rmse_endo),
                                                                                   np.var(CT_all_rmse_endo),
                                                                                   np.average(CT_all_rmse_epi),
                                                                                   np.var(CT_all_rmse_epi)))
    print("rvd_endo  {:.3f}  {:.6f}, Val_rvd_epi  {:.3f}  {:.6f}".format(np.average(CT_all_rvd_endo),
                                                                                   np.var(CT_all_rvd_endo),
                                                                                   np.average(CT_all_rvd_epi),
                                                                                   np.var(CT_all_rvd_epi)))
    ...
    for MR_patients in MR_test_list:
        ...
        x_list_MR = np.squeeze(np.concatenate((np.arange(0, w_MR - args.patch_size, args.patch_size // args.spacing)[:, np.newaxis],
                                            np.array([w_MR - args.patch_size])[:, np.newaxis])).astype(int))
        y_list_MR = np.squeeze(np.concatenate((np.arange(0, h_MR - args.patch_size, args.patch_size // args.spacing)[:, np.newaxis],
                                            np.array([h_MR - args.patch_size])[:, np.newaxis])).astype(int))

        if d_MR <= args.patch_size:
            z_list_MR = [0]
        else:
            z_list_MR = np.squeeze(np.concatenate((np.arange(0, d_MR - args.patch_size, args.patch_size // 4)[:, np.newaxis],
                                                np.array([d_MR - args.patch_size])[:, np.newaxis])).astype(int))
        start_time = time.time()
        ...
        for z in z_list_MR:
            for x in x_list_MR:
                for y in y_list_MR:
                    ...
                    prediMR_MR[:, z:z + args.patch_size, x:x + args.patch_size, y:y + args.patch_size] += model.segmentor(
                        preout_MR, 'test').squeeze().cpu().data.numpy()
        ...
        # nib.save(prediMR_nii, os.path.join(save_path, '{}/prediMR.nii.gz'.format(test_list[idx])))

        s += 1
        print("length_MR:{}".format(s))
        print("{} Val_dice_endo  {:.3f}, Val_dice_epi  {:.3f},  Dice: {:.3f}".format(MR_patients, dice[1], dice[2],
                                                                                  (dice[1] + dice[2]) / 2))
        print(" Val_iou_endo  {:.3f}, Val_iou_epi  {:.3f},  IOU: {:.3f}".format( iou[1], iou[2],
                                                                                  (iou[1] + iou[2]) / 2))
        print(" Val_hd_endo  {:.3f}, Val_hd_epi  {:.3f},  HD: {:.3f}".format( hd[1], hd[2],
        ...
        # print("[{}] Testing Finished, Cost {:.2f}s, ".format(idx, time.time()-start_time))
    print("ALL MR :dice_endo  {:.3f}  {:.6f}, Val_dice_epi  {:.3f}  {:.6f}".format(np.average(MR_all_dice_endo), np.var(MR_all_dice_endo),
                                                                   np.average(MR_all_dice_epi),np.var(MR_all_dice_epi)))
    print("iou_endo  {:.3f}  {:.6f}, Val_iou_epi  {:.3f}  {:.6f}".format(np.average(MR_all_iou_endo), np.var(MR_all_iou_endo),
                                                                   np.average(MR_all_iou_epi),np.var(MR_all_iou_epi)))
    print("hd_endo  {:.3f}  {:.6f}, Val_hd_epi  {:.3f}  {:.6f}".format(np.average(MR_all_hd_endo), np.var(MR_all_hd_endo),
                                                                   np.average(MR_all_hd_epi),np.var(MR_all_hd_epi)))
    print("rmse_endo  {:.3f}  {:.6f}, Val_rmse_epi  {:.3f}  {:.6f}".format(np.average(MR_all_rmse_endo), np.var(MR_all_rmse_endo),
                                                                   np.average(MR_all_rmse_epi),np.var(MR_all_rmse_epi)))

    print("rvd_endo  {:.3f}  {:.6f}, Val_rvd_epi  {:.3f}  {:.6f}".format(np.average(MR_all_rvd_endo), np.var(MR_all_rvd_endo),
                                                                   np.average(MR_all_rvd_epi),np.var(MR_all_rvd_epi)))
