...
N_train_patients = 5  # 100		# number of patients to use from train set
N_test_patients = 1  # 54		# number of patients to use from the test set
...
for i in range(N_test_patients):
    ...
    create_folder(
        os.path.join("UTAH Test set", str(i + 1) + " - " + test_files[i], "cavity"))  # folder for CAVITY labels
    create_folder(os.path.join("UTAH Test set", str(i + 1) + " - " + test_files[i],
    ...
    for n_slice in range(patient_3DMRI_scan.shape[2]):
        ...
        cv2.imwrite(os.path.join("UTAH Test set", str(i + 1) + " - " + test_files[i], "data", output_filename),
                    patient_3DMRI_scan[:, :, n_slice])
        cv2.imwrite(os.path.join("UTAH Test set", str(i + 1) + " - " + test_files[i], "cavity", output_filename),
                    patient_3DMRI_cavity[:, :, n_slice] * 255)
