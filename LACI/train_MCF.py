...
parser.add_argument('--exp', type=str,  default="MCF_flod0", help='model_name')
...
if __name__ == "__main__":
    ...
    shutil.copy('/data/chenjinfeng/code/semi_supervised/All_code/code_all/train_MCF.py', snapshot_path + '/code')
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            v_loss_seg_dice = losses.Binary_dice_loss(v_outputs_soft[:labeled_bs, 1, :, :, :], v_label[:labeled_bs] == 1)

            r_loss_seg = F.cross_entropy(r_outputs[:labeled_bs], r_label[:labeled_bs])
            r_outputs_soft = F.softmax(r_outputs, dim=1)
            r_loss_seg_dice = losses.Binary_dice_loss(r_outputs_soft[:labeled_bs, 1, :, :, :], r_label[:labeled_bs] == 1)
