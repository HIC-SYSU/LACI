...
parser.add_argument('--exp', type=str,  default="Comwin_flod0", help='model_name')
...
parser.add_argument('--total_num', type=int,  default=80, help='maximum samples to train')
parser.add_argument('--patch_size', type=list, default=[112,112,80], help='crop size of input image')
...
### costs
parser.add_argument('--with_dice', type=str2bool,  default=True, help='with_dice loss')     ## 控制模型是否在损失计算时计算dice
...
patch_size = args.patch_size#(96, 96, 96)
...
if __name__ == "__main__":
    ...
    def pseudo_labeling_from_most_confident_prediction(pred_1, pred_2, max_1, max_2):
        ...
        max_conf_each_branch_ex_3, _ = torch.max(prob_all_ex_3, dim=1)  # bs, n_branch - 1, h, w, d     ## 两个分支在类别上的最大值
        max_conf_ex_3, branch_id_max_conf_ex_3 = torch.max(max_conf_each_branch_ex_3, dim=1, keepdim=True)  # bs, h, w, d  ## 哪个分支的概率更大
        #### 对概率最大索引进行操作
        max_all_ex_3 = torch.stack([max_1, max_2], dim=1)  # bs, n_branch - 1, h, w, d
        ## 根据置信度选择伪标签
        pseudo_12 = torch.gather(max_all_ex_3, dim=1, index=branch_id_max_conf_ex_3)[:, 0]  ## max_all_ex_3两个分支各自的预测最大值，branch_id_max_conf_ex_3两个分支一块的预测最大哦值 - - -根据branch_id_max_conf_ex_3指示从哪一个分支抽取值
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            unlabeled_volume_batch, unlabel_label_batch = images[:labeled_bs], labels[:labeled_bs]
            labeled_volume_batch, label_label_batch = images[labeled_bs:], labels[labeled_bs:]
        ...
        #     labeled_volume_batch, label_label_batch = labeled_sampled_batch['image'], labeled_sampled_batch['label']
            ...
            forward_step_num = scale_num * 2 - 1        ##为什么scale_num=2
            ...
            ### 将输出的概率结果out_at8进行拼接， x1时特征，x8是输出前的一层
            ...
            _, max_la_1 = torch.max(logits_sup_1_list[scale_id], dim=1)     ## logits_sup_1_list[scale_id]是out_at8_sup_1，他的dim=1是numclasses，返回的第一个值是最大值，第二个值是索引
            ...
            pred_sup_1 = F.softmax(logits_sup_1_list[scale_id], dim=1)      ## 将输出的原始分布转化为概率分布
            ...
            """gather all losses"""
            cps_la_weight = get_unsup_cont_weight(iter_num // 150, args.cps_la_weight_final, scheme=args.cps_la_rampup_scheme, ramp_up_or_down=args.cps_la_rampup)
            cps_un_weight = get_unsup_cont_weight(iter_num // 150, args.cps_un_weight_final, scheme=args.cps_un_rampup_scheme, ramp_up_or_down=args.cps_un_rampup)
            ...
            if iter_num % 1000 == 0:
                ...
                torch.save({'model': model.state_dict(), 'max_iterations': max_iterations}, save_mode_path)
