...
from networks.unet_3D_UAMT import unet_3D
...
parser.add_argument('--model', type=str,
                    default='unet_3D_UAMT', help='model_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            preds = torch.zeros([stride * T, 3, d, w, h]).cuda()
            for i in range(T//2):
                ema_inputs = volume_batch_r + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)
                with torch.no_grad():
                    preds[2 * stride * i:2 * stride * (i + 1)] = ema_model(ema_inputs)
            preds = torch.softmax(preds, dim=1)
            preds = preds.reshape(T, stride, 3, d, w, h)
            ...
            consistency_dist = losses.softmax_mse_loss(outputs[args.labeled_bs:], ema_output)  # (batch, 2, 112,112,80)
