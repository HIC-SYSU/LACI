...
from networks.vnet_DAE import VNetDAE
...
parser.add_argument('--exp', type=str,  default='AU_flod0', help='model_name')
...
parser.add_argument('--max_iterations', type=int,  default=8000, help='maximum epoch number to train')      ## 论文中说是6k？
...
snapshot_path = snapshot_path = "/data/chenjinfeng/code/semi_supervised/All_code/code_all/weights/{}/{}/{}".format(args.dataset, args.exp, args.nb_labels)
...
if __name__ == "__main__":
    ...
    #     shutil.rmtree(snapshot_path + '/code')
    #
    # shutil.copy('/data/chenjinfeng/code/semi_supervised/All_code/code_all/train_DAE.py', snapshot_path + '/code')
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # noisy_batch += noise
            ### 数据处理中没有noisy_label，所以注释掉了，换成以下对image加噪
            noise = torch.clamp(torch.randn_like(volume_batch) * 0.1, -0.2, 0.2)
            ema_inputs = volume_batch + noise

            outputs, emb = model(ema_inputs)  # 4*64* 3, 256
