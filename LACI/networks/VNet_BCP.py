...
class VNet_BCP(nn.Module):
    def __init__(self, n_channels=3, n_classes=4, n_filters=16, normalization='none', has_dropout=False, has_residual=False):
        super(VNet_BCP, self).__init__()
...
class VNet(nn.Module):
    def __init__(self, n_channels=1, n_classes=4, base_filters=16):
        super(VNet, self).__init__()

        # Encoder
        self.block1 = ConvBlock_o(n_channels, base_filters, num_convs=1)
        self.down1  = nn.Conv3d(base_filters, base_filters*2, kernel_size=2, stride=2)

        self.block2 = ConvBlock_o(base_filters*2, base_filters*2, num_convs=2)
        self.down2  = nn.Conv3d(base_filters*2, base_filters*4, kernel_size=2, stride=2)

        self.block3 = ConvBlock_o(base_filters*4, base_filters*4, num_convs=3)
        self.down3  = nn.Conv3d(base_filters*4, base_filters*8, kernel_size=2, stride=2)

        self.block4 = ConvBlock_o(base_filters*8, base_filters*8, num_convs=3)
        self.down4  = nn.Conv3d(base_filters*8, base_filters*16, kernel_size=2, stride=2)

        self.block5 = ConvBlock_o(base_filters*16, base_filters*16, num_convs=3)

        # Decoder
        self.up4 = nn.ConvTranspose3d(base_filters*16, base_filters*8, kernel_size=2, stride=2)
        self.block6 = ConvBlock_o(base_filters*8, base_filters*8, num_convs=3)

        self.up3 = nn.ConvTranspose3d(base_filters*8, base_filters*4, kernel_size=2, stride=2)
        self.block7 = ConvBlock_o(base_filters*4, base_filters*4, num_convs=3)

        self.up2 = nn.ConvTranspose3d(base_filters*4, base_filters*2, kernel_size=2, stride=2)
        self.block8 = ConvBlock_o(base_filters*2, base_filters*2, num_convs=2)

        self.up1 = nn.ConvTranspose3d(base_filters*2, base_filters, kernel_size=2, stride=2)
        self.block9 = ConvBlock_o(base_filters, base_filters, num_convs=1)

        self.out_conv = nn.Conv3d(base_filters, n_classes, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x5 = self.block5(self.down4(x4))

        # Decoder（带残差相加 skip）
        x5_up = self.up4(x5) + x4
        x6 = self.block6(x5_up)

        x6_up = self.up3(x6) + x3
        x7 = self.block7(x6_up)

        x7_up = self.up2(x7) + x2
        x8 = self.block8(x7_up)

        x8_up = self.up1(x8) + x1
        x9 = self.block9(x8_up)
