...
import numpy as np
from unetWithArgs_ACTION import UNet
from vnet_comwin import VNet
...
def create_model(ema=False, num_classes=4, train_encoder=True, train_decoder=True):
        # Network definition
        # model = net_factory(net_type='unet', in_chns=1, class_num=num_classes, train_encoder=train_encoder, train_decoder=train_decoder)
        model = UNet(in_chns=1, class_num=num_classes, train_encoder=train_encoder, train_decoder=train_decoder, unfreeze_seg=True).cuda()
        if ema:
...
def create_model_3d(ema=False, num_classes=4):
    # Network definition
    # model = net_factory_3d(net_type="vnet", in_chns=1, class_num=num_classes)
...
class ISD_3d(nn.Module):
    def __init__(self, K=48, m=0.99, Ts=0.1, Tt = 0.01, num_classes=4, train_encoder=True, train_decoder=True, 
        ...
        self.register_buffer('queue_mask', torch.randn(self.K, 512, num_classes * output_pooling_size ** 3))
    ...
    @torch.no_grad()
    def _momentum_update_key_encoder(self):     # 动量更新：平滑更新权重
        for param_q, param_k in zip(self.model.parameters(), self.ema_model.parameters()):      # param_q主模型权重，param_k动量模型权重，m是权重系数
    ...
    def forward(self, im_q, im_k, Ts=None, Tt=None):
        ...
        outputs, latent_vector, feat_map_stu = self.model(im_q) 
        outputs_tmp = outputs

        with torch.no_grad():
            ema_inputs = im_k 
            ema_output_tmp, ema_latent_tmp, feat_map_tea = self.ema_model(ema_inputs)

        ############################################# calculate the KLD with anchors  #################################
        with torch.no_grad():
            # update the key encoder
            self._momentum_update_key_encoder()
            
            # shuffle keys
            shuffle_ids, reverse_ids = get_shuffle_ids(im_k.shape[0])       # 生成前向的打乱索引和反向的恢复索引，这种随机打乱操作常使用于跨设备的样本混洗，打破每个设备数据的独立性，增强模型的泛化性能
        ...
        queue = self.queue.clone().detach() # torch.Size([48, 256])
        queue_mask = self.queue_mask.clone().detach().transpose(0, 1).contiguous() # torch.Size([36, 48, 16384])
        ...
        ## 滑动窗口，基于patchsize将输出特征切成小块，对每个小块使用模型的输出头和输出预测器
        ...
        output_tea_after_head = torch.cat(output_tea_after_head).reshape(batch_size, -1, 
                                        ...
                                        output_tea_after_head[0].shape[4], ).contiguous() 
        
        # print(f'ema_latent_vector:{ema_latent_vector.shape}')
        desired_compressed_lat_k  = self.k_latent_head(ema_latent_vector) 
        desired_compressed_lat_q = self.latent_predictor(self.q_latent_head(latent_vector)) 
        ...
        # compute the 4 logits 计算潜在向量的对比相似度
        # print(f'output_tea_after_head:{output_tea_after_head.shape},queue_mask:{queue_mask.shape}')
        # print(f'desired_compressed_lat_q:{output_tea_after_head.shape},queue:{queue.shape}')
        ...
        # dequeue and enqueue 队列更新，更新潜在向量的队列和特征图队列，将最新的特征加入到队列中，移除旧的特征
