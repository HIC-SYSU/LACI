...
from .losses import DICELoss, mask_DiceLoss, mask_DiceLoss_ABDO

# DICE = mask_DiceLoss(nclass=3)
# DICE = DICELoss(nclass=4)
CE = nn.CrossEntropyLoss(reduction='none')
DICE = mask_DiceLoss_ABDO(nclass=4)



def context_mask(img, mask_ratio, size):
    ...
    w = np.random.randint(0, size[0] - patch_pixel_x)
    h = np.random.randint(0, size[1] - patch_pixel_y)
    z = np.random.randint(0, size[2] - patch_pixel_z)
...
def mix_loss(net3_output, img_l, patch_l, mask1, l_weight=1.0, u_weight=0.5, unlab=False):
    img_l, patch_l = img_l.type(torch.int64), patch_l.type(torch.int64)
    image_weight, patch_weight = l_weight, u_weight
    if unlab:
        image_weight, patch_weight = u_weight, l_weight
    # patch_mask = 1 - mask
    valid_mask_img = (img_l != 255).float()
    valid_mask_patch = (patch_l != 255).float()
    # print(mask1.shape, valid_mask_img.shape, valid_mask_patch.shape)
    mask = mask1.float() * valid_mask_img
    patch_mask = (1 - mask1.float()) * valid_mask_patch
...
def mix_loss_or(net3_output, img_l, patch_l, mask, l_weight=1.0, u_weight=0.5, unlab=False):
    img_l, patch_l = img_l.type(torch.int64), patch_l.type(torch.int64)
    image_weight, patch_weight = l_weight, u_weight
    if unlab:
        image_weight, patch_weight = u_weight, l_weight
