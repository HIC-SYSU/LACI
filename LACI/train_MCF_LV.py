...
if __name__ == "__main__":
    ...
    if os.path.isdir(snapshot_path + '/code'):
        shutil.rmtree(snapshot_path + '/code')


    shutil.copy('/data/chenjinfeng/code/semi_supervised/All_code/code_all/train_MCF_LV.py', snapshot_path + '/code')
    ...
    labeled_idxs = list(range(args.labelnum))  # todo set labeled num
    unlabeled_idxs = list(range(args.labelnum, args.max_samples))  # todo set labeled num all_sample_num
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch1, volume_label1 = sampled_batch['image'], sampled_batch['label']
            volume_batch2, volume_label2 = sampled_batch['image'], sampled_batch['label']
            ...
            v_loss_seg_dice = losses.Binary_dice_loss(v_outputs_soft[:labeled_bs, 1, :, :, :], v_label[:labeled_bs] == 1)+losses.Binary_dice_loss(v_outputs_soft[:labeled_bs, 2, :, :, :], v_label[:labeled_bs] == 2)

            r_loss_seg = F.cross_entropy(r_outputs[:labeled_bs], r_label[:labeled_bs])
            r_outputs_soft = F.softmax(r_outputs, dim=1)
            r_loss_seg_dice = losses.Binary_dice_loss(r_outputs_soft[:labeled_bs, 1, :, :, :], r_label[:labeled_bs] == 1)+ losses.Binary_dice_loss(r_outputs_soft[:labeled_bs, 2, :, :, :], r_label[:labeled_bs] == 2)
            ...
            # 对类别1进行计算
            diff_mask_1 = ((v_predict == 1) ^ (r_predict == 1)).to(torch.int32)
            # 对类别2进行计算
            diff_mask_2 = ((v_predict == 2) ^ (r_predict == 2)).to(torch.int32)
            # 合并两个掩码，表示预测在类别1或类别2上的不一致
            diff_mask = diff_mask_1 | diff_mask_2
            # diff_mask = ((v_predict == 1) ^ (r_predict == 1)).to(torch.int32)           ## 识别预测结果的差异，用于识别预测不一致的体素。
            ...
            r_mse      = torch.sum(diff_mask * r_mse_dist) / (torch.sum(diff_mask) + 1e-16)     ## 只计算预测不一致的位置的损失

            v_supervised_loss =  (v_loss_seg + v_loss_seg_dice) + 0.5 * v_mse
            r_supervised_loss =  (r_loss_seg + r_loss_seg_dice) + 0.5 * r_mse       ## 公示8.监督损失

            #### 伪标签的生成:从非标签数据部分 (labeled_bs:) 对 v_outputs_soft 和 r_outputs_soft 应用温度缩放 (T) 和正规化，生成伪标签 (Plabel)。这是自学习或半监督学习中常见的技术，通过已有的模型输出为未标记的数据生成标签。
            ...
            if Good_student == 0:
                r_consistency_dist = consistency_criterion(r_outputs_soft[labeled_bs:, :, :, :, :], Plabel)         ## 公式9，无监督损失
